...
The goal of this project is to produce a low cost alphanumeric display readable
from ~50ft outdoors in bright sunlight.
...
scan of 7.5KHz. An ARM Cortex M0 drives push-pull shift registers to control the
columns using its SPI peripheral, and switches the rows with dedicated GPIOs.

The board is programmed via a TTL UART, with a 48 byte LSB-first row major
representation of the pixels on the board, left to right. The [driver.py](driver.py)
script runs the display.

The font in [ledfont.py](ledfont.py) is the 4x6 terminal font from the Linux kernel.
...
Revision 2.1 will use cheaper and more widely available LEDs.
